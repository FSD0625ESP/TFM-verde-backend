...
const register = async (req, res) => {
    ...
    try {
        ...
        jwt.sign({ id: savedUser._id }, process.env.JWT_SECRET, { expiresIn: '1d' }, (err, token) => {
            if (err) {
                console.error(err);
            ...
            if (!safeUser.profileImage) safeUser.profileImage = '/images/default-avatar.png';
            ...
            res.status(201).send({ msg: "User registered successfully", user: safeUser });
        });
    } catch (error) {
        console.error(error);
...
const googleLogin = async (req, res) => {
    try {
        ...
        jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1d' }, (err, token) => {
            ...
            delete safeUser.password;
            return res.status(200).send({ msg: 'Login con Google exitoso', user: safeUser });
        });
    } catch (error) {
        console.error(error);
        return res.status(400).send({ msg: error.message || 'Error al verificar token de Google' });
    }
};
...
const verifyForgotPasswordToken = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ email });
        if (!user) return res.status(404).send({ msg: 'No se encontro el usuario' });
        // TODO: Aquí generarías un token y enviarías un email al usuario con un enlace para restablecer la contraseña 
        res.status(200).send({ msg: 'Se ha enviado un correo para restablecer la contraseña' });
    } catch (error) {
        console.error(error);
        return res.status(400).send({ msg: error.message });
    }
